...
const struct device *dirX = NULL;
const struct device *stepX = NULL;
const struct device *enableX = NULL;

const struct device *dirY = NULL;
const struct device *stepY = NULL;
const struct device *enableY = NULL;

const struct device *dirZ = NULL;
const struct device *stepZ = NULL;
const struct device *enableZ = NULL;

#ifdef ENABLE_DUAL_AXIS
const struct device *dirXdual = NULL;
const struct device *stepXdual = NULL;
const struct device *enableXdual = NULL;

const struct device *dirYdual = NULL;
const struct device *stepYdual = NULL;
const struct device *enableYdual = NULL;

const struct device *dirZdual = NULL;
const struct device *stepZdual = NULL;
const struct device *enableZdual = NULL;
#endif
...
void st_go_idle ()
{
        ...
        bool disableMotors = false; // Keep enabled.

        if (((settings.stepper_idle_lock_time != 0xff) || sys_rt_exec_alarm || sys.state == STATE_SLEEP) && sys.state != STATE_HOMING) {
                // Force stepper dwell to lock axes for a defined amount of time to ensure the axes come to a complete
                // stop and not drift from residual inertial forces at the end of the last movement.
                delay_ms (settings.stepper_idle_lock_time);
                disableMotors = true; // Override. Disable steppers.
        }

        st_enable_motors (!disableMotors);
}
...
#ifdef ENABLE_DUAL_AXIS
                        st.dir_outbits_dual = st.exec_block->direction_bits_dual;
#endif
...
void TIMER0_OVF_vect ()
{
        ...
        gpio_pin_set (stepX, MOTORX_STEP_PIN, false);
        gpio_pin_set (stepY, MOTORY_STEP_PIN, false);
        gpio_pin_set (stepZ, MOTORZ_STEP_PIN, false);

#ifdef ENABLE_DUAL_AXIS
        gpio_pin_set (stepXdual, MOTORX_STEP_PIN, false);
        gpio_pin_set (stepYdual, MOTORY_STEP_PIN, false);
        gpio_pin_set (stepZdual, MOTORZ_STEP_PIN, false);
#endif
...
void st_reset ()
{
        ...
        gpio_pin_set (dirZ, MOTORZ_DIR_PIN, false);
...
void stepper_init ()
{
        ...
        if (dirX == NULL || gpio_pin_configure (dirX, MOTORX_DIR_PIN, GPIO_OUTPUT_ACTIVE | MOTORX_DIR_FLAGS) < 0) {
                LOG_ERR ("dirX pin configuration error");
                return;
        }

        stepX = device_get_binding (MOTORX_STEP_LABEL);

        if (stepX == NULL || gpio_pin_configure (stepX, MOTORX_STEP_PIN, GPIO_OUTPUT_ACTIVE | MOTORX_STEP_FLAGS) < 0) {
                LOG_ERR ("stepX pin configuration error");
                return;
        }

        enableX = device_get_binding (MOTORX_ENABLE_LABEL);

        if (enableX == NULL || gpio_pin_configure (enableX, MOTORX_ENABLE_PIN, GPIO_OUTPUT_ACTIVE | MOTORX_ENABLE_FLAGS) < 0) {
                LOG_ERR ("enableX pin configuration error");
                return;
        }
        ...
        if (dirY == NULL || gpio_pin_configure (dirY, MOTORY_DIR_PIN, GPIO_OUTPUT_ACTIVE | MOTORY_DIR_FLAGS) < 0) {
                LOG_ERR ("dirX pin configuration error");
                return;
        }
        ...
        if (stepY == NULL || gpio_pin_configure (stepY, MOTORY_STEP_PIN, GPIO_OUTPUT_ACTIVE | MOTORY_STEP_FLAGS) < 0) {
                LOG_ERR ("stepX pin configuration error");
                return;
        }
        ...
        if (enableY == NULL || gpio_pin_configure (enableY, MOTORY_ENABLE_PIN, GPIO_OUTPUT_ACTIVE | MOTORY_ENABLE_FLAGS) < 0) {
                LOG_ERR ("enableX pin configuration error");
                return;
        }
...
void st_prep_buffer ()
{
        ...
        while (segment_buffer_tail != segment_next_head) { // Check if we need to fill the buffer.
                        ...
                        if (sys.step_control & STEP_CONTROL_EXECUTE_HOLD) { // [Forced Deceleration to Zero Velocity]
                                ...
                                if (pl_block->entry_speed_sqr > nominal_speed_sqr) { // Only occurs during override reductions.
                                        ...
                                        if (prep.accelerate_until <= 0.0) { // Deceleration-only.
                                                ...
                                                prep.decelerate_after = inv_2_accel
                                        ...
                                        if (intersect_distance < pl_block->millimeters) { // Either trapezoid or triangle types
                                                ...
                                                if (prep.decelerate_after < intersect_distance) { // Trapezoid type
                                                        ...
                                                        if (pl_block->entry_speed_sqr == nominal_speed_sqr) {
                                                                ...
                                                                prep.accelerate_until
                                                                        -= inv_2_accel * (nominal_speed_sqr - pl_block->entry_speed_sqr);
                                                        }
                                                }
                                                else { // Triangle type
                ...
                */
                ...
                uint32_t cycles
                        = ceil ((TICKS_PER_MICROSECOND * 1000000 * 60) * inv_rate); // (cycles/step)
...
#else
                                                                                    // Compute step timing and timer prescalar for normal step
                                                                                    // generation.
